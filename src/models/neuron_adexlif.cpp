...
#include "neuron_adexlif.h"

using namespace std;


neuron_adexlif::neuron_adexlif(QWidget *parent, int NumberNeuronsGroup, QString label, float posX, float posY, QString ip, int idGlobalNeuron, int typeNeuron, int localRemote, Parameters *p, double w_default, QString fx,
...
void neuron_adexlif::sendDataToGeneralMonitor(bool spiking){
...
void neuron_adexlif::paintLocalMonitor(){
    if (!monitor)
...
void neuron_adexlif::calculateIinh() {
   if (Iinh_enabled) {
      Iinh_prior=p->Iinh;
...
void neuron_adexlif::calculateIexc(){
   if (Iexc_enabled) {
      Iexc_prior=p->Iexc;
...
void neuron_adexlif::calculateV() {
       V_prior=p->V;
...
void neuron_adexlif::calculateValues(){
    bool spiking=false;
...
double neuron_adexlif::get_IexcCurrent(){
    return IexcCurrent;
}
double neuron_adexlif::get_IinhCurrent(){
    return IinhCurrent;
}
double neuron_adexlif::get_VCurrent(){
   return VCurrent;
}
